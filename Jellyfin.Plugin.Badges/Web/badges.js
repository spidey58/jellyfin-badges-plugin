(function () {
    ...
    function makeRatingBadge(score, logoKey) {
        var wrapper = document.createElement('span');
    ...
    function getVideoBadges(stream) {
        ...
        if (vrt.indexOf('DOVI') === 0) {
            ...
            if (!CONFIG.suppressHdrWithDolbyVision) {
                if (vrt.indexOf('HDR10PLUS') !== -1) { if (CONFIG.enabled.hdr10plus) badges.push({ text: 'HDR10+', styleKey: 'hdr10plus' }); }
                else if (vrt.indexOf('HDR10') !== -1) { if (CONFIG.enabled.hdr10) badges.push({ text: 'HDR10', styleKey: 'hdr10' }); }
            }
    ...
    function fetchNfoRating(path, source, callback) {
        var api = window.ApiClient;
        if (!api || !path) { callback(null); return; }
        api.ajax({
            url: api.getUrl('Badges/nfoRating', { path: path, source: source }),
            type: 'GET',
            dataType: 'json'
        }).then(function (result) {
            callback(result && typeof result.rating === 'number' ? result.rating : null);
        }).catch(function (e) {
            log('fetchNfoRating error:', e);
            callback(null);
        });
    }
    ...
    function injectBadges() {
        ...
        fetchItem(itemId, function (item) {
            ...
            if (CONFIG.enabled.imdb && item.CommunityRating) {
                row.appendChild(makeRatingBadge(item.CommunityRating, 'imdb'));
            }
            ...
            // TMDb rating isn't exposed by Jellyfin's API as a distinct
            // field (only a single collapsed CommunityRating), so it's
            // fetched separately by reading the NFO directly and appended
            // once it resolves, rather than blocking the rest of the row.
            if (CONFIG.enabled.tmdb && source.Path) {
                fetchNfoRating(source.Path, 'tmdb', function (tmdbRating) {
                    if (tmdbRating === null) return;
                    // Row may have been replaced by a newer navigation by now
                    if (!document.body.contains(row)) return;
                    row.appendChild(makeRatingBadge(tmdbRating, 'tmdb'));
                });
            }
        });
    }
