...
namespace Jellyfin.Plugin.Badges.Services
{
    ...
    ///
    /// This is the preferred injection method because it works regardless
    /// of file permissions - a common failure point in Docker deployments
    /// where the web root is owned by a different user than the Jellyfin
    /// process. If File Transformation is not installed, the caller should
    /// fall back to direct file writes.
    ///
    /// Jellyfin loads each plugin into its own collectible
    /// AssemblyLoadContext, so File Transformation's types cannot be
    /// referenced directly at compile time - everything here goes through
    /// reflection, matching the pattern documented in that plugin's README.
    /// </summary>
    public static class FileTransformationIntegration
    {
        ...
        /// <summary>
        /// Attempts to register our transformation with the File
        /// Transformation plugin, if it is installed and loaded.
        /// </summary>
        /// <returns>True if registration succeeded, false if the plugin
        /// was not found or registration failed for any reason.</returns>
        ...
        /// <summary>
        /// Invoked by the File Transformation plugin via reflection every
        /// time index.html is served. Receives an object with a
        /// "contents" property holding the current file content, and
